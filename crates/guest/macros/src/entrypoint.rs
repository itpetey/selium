...
pub fn expand(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let call_user = if f.sig.asyncness.is_some() {
        quote! { selium_guest::block_on(#user_ident(#(#arg_idents),*)) }
    } else {
    ...
    entrypoint_inputs.extend(params.iter().flat_map(|param| match &param.kind {
        ...
        ParamKind::SplitInt {
            ...
            parse_quote! { #low_ident: selium_guest::abi::GuestInt },
            parse_quote! { #high_ident: selium_guest::abi::GuestInt },
        ],
    }));
            ...
            ParamKind::Decode { ptr_ident, len_ident } => {
                ...
                let decode_value = if is_str_type(ty) {
                    ...
                    quote! {
                        ...
                        match selium_guest::abi::decode_rkyv::<#ty>(bytes) {
                            Ok(decoded) => decoded,
                            Err(err) => panic!(
                                "failed to decode entrypoint argument {}: {}",
                                stringify!(#ident),
