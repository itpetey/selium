...
use selium_runtime_support::{
    ClientTlsPaths, ServerTlsPaths, build_quic_client_config as build_support_quic_client_config,
    build_quic_server_endpoint as build_support_quic_server_endpoint,
    build_tls_acceptor as build_support_tls_acceptor,
    build_tls_connector as build_support_tls_connector, derive_server_name, parse_authority_addr,
...
fn build_quic_server_endpoint(binding: &NetworkIngressBinding) -> Result<Endpoint> {
    ...
    build_support_quic_server_endpoint(
        addr,
        ServerTlsPaths {
            cert: &binding.cert_path,
            key: &binding.key_path,
        },
        None,
    )
}
...
fn build_quic_client_config(profile: &NetworkEgressProfile) -> Result<quinn::ClientConfig> {
    build_support_quic_client_config(ClientTlsPaths {
        ca_cert: &profile.ca_cert_path,
        client_cert: profile.client_cert_path.as_deref(),
        client_key: profile.client_key_path.as_deref(),
    })
}

fn build_tls_connector(profile: &NetworkEgressProfile) -> Result<TlsConnector> {
    build_support_tls_connector(ClientTlsPaths {
        ca_cert: &profile.ca_cert_path,
        client_cert: profile.client_cert_path.as_deref(),
        client_key: profile.client_key_path.as_deref(),
    })
...
fn build_tls_acceptor(binding: &NetworkIngressBinding) -> Result<TlsAcceptor> {
    build_support_tls_acceptor(ServerTlsPaths {
        cert: &binding.cert_path,
        key: &binding.key_path,
    })
}
