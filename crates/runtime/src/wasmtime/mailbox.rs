...
impl GuestMailbox {
    ...
    fn enqueue(&self, task_id: usize) {
        ...
        unsafe {
            ...
            let Ok(id) = GuestUint::try_from(task_id) else {
                return;
            };
...
impl WakeMailbox for GuestMailbox {
    fn refresh_base(&self, base: usize) {
        GuestMailbox::refresh_base(self, base);
    }

    fn close(&self) {
        GuestMailbox::close(self);
    }

    fn waker(&'static self, task_id: usize) -> std::task::Waker {
        GuestMailbox::waker(self, task_id)
    }

    fn is_closed(&self) -> bool {
        GuestMailbox::is_closed(self)
    }

    fn is_signalled(&self) -> bool {
        GuestMailbox::is_signalled(self)
    }

    fn wait_for_signal<'a>(&'a self) -> Pin<Box<dyn Future<Output = ()> + Send + 'a>> {
        Box::pin(GuestMailbox::wait_for_signal(self))
    }
}
