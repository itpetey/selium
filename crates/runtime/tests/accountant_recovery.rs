...
fn accounting_descriptor(
    ...
    SystemGuestDescriptor {
        ...
        grants,
        dependencies,
        readiness: selium_runtime::ReadinessCondition::ActivityLogContains("guest ready".to_string()),
        tenant: None,
        serving_role: None,
        handlers: Vec::new(),
    }
...
fn assert_enforcement_reauthored(runtime: &Runtime) {
    ...
    for class in [ResourceClass::SharedRegion, ResourceClass::DurableLog, ResourceClass::BlobStore]
    {
        assert_eq!(
...
async fn accountant_ledger_survives_restart_and_reauthors_enforcement() {
    ...
    let respawn = respawn_report
        .guests
        .first()
        .expect("respawned accountant");
