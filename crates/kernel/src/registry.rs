...
impl InstanceRegistry {
    ...
    pub fn load_mailbox(&mut self, mb: &'static dyn WakeMailbox) -> Result<(), RegistryError> {
    ...
    pub fn set_memory_limit(&mut self, bytes: usize) -> Result<(), RegistryError> {
        self.with_instance_state(|state| {
            state.memory_limit_bytes = Some(bytes);
        })
        .ok_or(RegistryError::MissingInstance)
    }

    /// Read the configured memory limit for this instance.
    pub fn memory_limit(&self) -> Result<Option<usize>, RegistryError> {
        self.with_instance_state(|state| state.memory_limit_bytes)
            .ok_or(RegistryError::MissingInstance)
    }
    ...
    pub fn resolve_future_handle(&self, handle: usize) -> Option<ResourceId> {
        let handles = self.registry.handles.lock().ok()?;
    ...
    pub fn mailbox(&self) -> Option<&'static dyn WakeMailbox> {
    ...
    pub fn future_state(&self, handle: usize) -> Option<GuestFuture> {
