...
use crate::{
    ...
    io::{
        ...
        channels::{StrongReader, WeakWriter},
    },
...
pub struct UdpSocket {
    pub(super) recv_reader: StrongReader,
    pub(super) recv_signal: Signal,
    pub(super) send_writer: WeakWriter,
    pub(super) send_signal: Signal,
...
fn attach_udp_channels(shared_id: u64) -> Result<UdpChannelLayout> {
    ...
    let writer_id = send_region
        .allocate_writer_id()
        .map_err(|e| GuestError::Host(e.to_string()))?;
    let send_writer = WeakWriter::new(send_region, writer_id, Some(send_signal.clone()));
