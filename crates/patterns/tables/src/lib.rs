//! Selium live tables pattern crate.
//!
//! Provides a materialised `LiveTable<K, V>` projected from a pub/sub topic
//! stream. Writes are published as `LiveTableMessage`s; reads are served from
//! a local `HashMap`. Remote writes from other processes attached to the
//! same topic are picked up by calling [`LiveTable::sync`].
        ...
        > + rkyv::bytecheck::CheckBytes<rkyv::api::high::HighValidator<'a, rkyv::rancor::Error>>,
{
    ...
    pub fn attach(region_id: u64, capacity: u64) -> Result<Self> {
        let (publisher, subscriber) = pubsub::attach_pair(region_id, capacity)?;
        let table = Self {
            publisher: RefCell::new(publisher),
            subscriber: RefCell::new(subscriber),
            local: RefCell::new(HashMap::new()),
