...
Discovery registration enables Selium guests and the runtime to register, revoke, and resolve URI-to-resource mappings through the discovery service, with runtime-authoritative ownership tracking, guest custom URI validation via ownership checks, and tenant-scoped resolution for process URIs.
