...
Define the unified addressing model for Selium: a single canonical address space rooted at `sel://<tenant>/<segments...>` with deterministic wire-name projection through path-label reversal and an advisory domain-to-tenant table, shared by QUIC, HTTP, and other edge protocols.
