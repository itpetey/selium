...
`selium-external-api` is a system guest that accepts user requests from an externally authenticated network path, parses them into `UserIntent`, decomposes intents into ordered `DelegatedInteraction` steps, and dispatches each to the appropriate system guest over RPC.
