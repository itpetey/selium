...
- [ ] 3.10 Write unit tests for `RpcClient` and `RpcConnection` using in-memory shared regions (native test fallback).
...
- [ ] 4.4 Update `selium-abi` tests to cover the new hostcall variants.

## 5. Guest Context and Entrypoint

- [ ] 5.1 Define `Context` struct in `selium-guest` with `discovery: RpcClient<DiscoveryRequest, DiscoveryResponse>` field and `from_raw(discovery_handle: u64) -> Result<Self>` constructor.
...
- [ ] 5.3 Update `#[entrypoint]` macro in `selium-guest-macros` to accept functions with an optional `Context` parameter. The macro SHALL decode the `Context` from the entry payload when present.
- [ ] 5.4 Write tests for `Context::from_raw` and `ResourceSender`/`ResourceListener` using native test fallbacks.
...
- [ ] 6.4 Implement `Context` construction in `selium-runtime` bootstrap: allocate session region, connect to discovery via `ResourceSender`, construct `Context`, and pass it to the guest entrypoint.
- [ ] 6.5 Write integration tests for hostcall dispatch: authorised send succeeds, unauthorised send returns capability error, recv returns connection info.

## 7. Discovery Wiring

- [ ] 7.1 Define `DiscoveryRequest` and `DiscoveryResponse` message types in `selium-discovery` with rkyv derive. `DiscoveryRequest` SHALL include `Resolve(String)` and other variants. `DiscoveryResponse` SHALL include `Found(ResourceTarget)` and `NotFound`.
- [ ] 7.2 Wire `discovery_main` entrypoint to accept `Context`, extract `ResourceListener`, and accept incoming RPC connections in a loop.
- [ ] 7.3 Implement request handling: deserialise `DiscoveryRequest`, route to `DiscoveryStore`, serialise `DiscoveryResponse`, reply via `RpcRequest::reply`.
- [ ] 7.4 Write tests for discovery RPC: resolve exact, resolve prefix, not found.

## 8. Final Validation

- [x] 8.1 Run `cargo test --workspace --all-targets` and fix any issues. **All 76 tests pass**.
- [ ] 8.2 Run `cargo clippy --workspace --all-targets -- -D warnings` and fix any issues.
- [ ] 8.3 Verify all spec scenarios have corresponding test coverage.